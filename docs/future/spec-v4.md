...
## API changes
- **Versioned routes**: Add `/api/v4/snapshot`, `/api/v4/health`, `/api/v4/models`. Preserve `/api/snapshot` and `/api/health` as v3 compatibility.
- **Modified response shapes**:
  - Snapshot response includes history metadata (`historyDays`, `snapshotCount`), deltas per model, capability tags, `family`, `parameters`, `license`, `tier`, `access`, and `scores.profile` (active weighting profile). Pagination support via `cursor` for `/api/v4/models` with `limit` and `filters` query params.
...
- **New behaviors**: Optional webhook handshake endpoint `/api/v4/webhooks/test` returning signed challenge; rate-limit headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`) on v4 routes.
...
- **Validation**: Unit fixtures covering known models to ensure profile weights produce expected ordering; deterministic scoring by rounding inputs before weighting; CI check to compare v3 vs v4 ordering deltas within tolerance.
...
- **API docs page**: New section for v4 routes, parameter examples, webhook signing guide, and rate-limit semantics.

## Migration notes (v3 → v4)
- Keep v3 endpoints untouched; introduce v4 under `/api/v4/*` with parallel responses.
