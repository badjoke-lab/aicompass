...
2. **must** — Add API routes with adapters preserving compatibility (`/api/snapshot`, `/api/health`, `/api/models`).
   - Touchpoints: `app/api/snapshot/route.ts`, `app/api/health/route.ts`, `app/api/models/route.ts`, shared serializers.
...
5. **must** — Enhance health reporting with queue stats, cache metrics, and HF quota usage.
   - Touchpoints: `lib/v4/health.ts`, `app/api/health/route.ts`, logging instrumentation.
...
9. **should** — Webhook subscription endpoint with signed delivery and delivery logs.
    - Touchpoints: `app/api/webhooks/route.ts`, `lib/v4/webhooks.ts`, logging/storage backend.
...
12. **could** — CSV export endpoint for snapshots with pagination-aware batching.
    - Touchpoints: `app/api/export/route.ts`, `lib/v4/export.ts`, streaming helpers.
